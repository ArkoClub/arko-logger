...
class Datetime(datetime_):  # noqa: N801
    def __format__(self, spec):
        ...
        rep = {
            ...
            "A": ("AM", "PM")[hour // 12],
            "Z": "%s%02d:%02d%s"
            % (sign, h, m, (":%09.06f" % s)[: 11 if s % 1 else 3] * (s > 0)),
            "ZZ": "%s%02d%02d%s"
            % (sign, h, m, ("%09.06f" % s)[: 10 if s % 1 else 2] * (s > 0)),
...
def aware_now():
    ...
    try:
        ...
        utc_naive = datetime_.fromtimestamp(timestamp, tz=timezone.utc).replace(
            tzinfo=None
        )
